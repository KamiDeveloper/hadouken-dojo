...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // --- CAMBIO PARA TRANSPARENCIA ---
    // Combinamos fuego y chispas
    vec3 finalColor = max(fire, sparks);
    
    // Calculamos el alpha (transparencia) basándonos en qué tan brillante es el píxel.
    // Tomamos el valor máximo de los canales R, G o B.
    float alpha = max(max(finalColor.r, finalColor.g), finalColor.b);
    
    // Aseguramos que el alpha esté entre 0.0 y 1.0 (a veces la intensidad pasa de 1.0)
    alpha = clamp(alpha, 0.0, 1.0);

    // El cuarto valor ahora es nuestra variable 'alpha' calculada en lugar de 1.0 fijo.
    fragColor = vec4(finalColor, alpha);
}
`;

/**
 * Componente de shaders con partículas flotantes y efecto de fuego (Sin humo, fondo transparente)
 * @param {Object} props - Props del componente
 * @param {string} [props.className] - Clases CSS adicionales
 * @param {number} [props.speed=1.0] - Velocidad del efecto
 * @param {number} [props.sparkSize=1.0] - Tamaño de las partículas
 * @param {number} [props.fireIntensity=1.0] - Intensidad del efecto de fuego
 * @param {React.Ref} ref - Referencia al div contenedor
 */
const SparksDriftingShaders = forwardRef(
    (
        {
            className,
            speed = 1.0,
        ...
        return (
            // Asegúrate de que el contenedor padre no tenga fondo negro si quieres verlo transparente
            <div
                ref={ref}
            ...
            >
                <Shader
                    ...
                    // react-shaders generalmente maneja el contexto WebGL con transparencia por defecto.
...
export default SparksDriftingShaders;